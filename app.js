...
/* ---------------------------------------------------------
   ...
   --------------------------------------------------------- */
const SUPABASE_URL = "https://yqvgjrjjlbcmuyhjvwgl.supabase.co";
...
const App = (() => {
  ...
  function renderLotes() {
    ...
    body.innerHTML = rows.map((l) => {
      ...
      return `<tr class="row-clickable" onclick="App.openDetail(${l.id})">
        ...
        <td class="row-actions"><button class="btn btn-icon" onclick="App.openDetail(${l.id})" aria-label="Ver lote"><i class="ti ti-eye" aria-hidden="true"></i></button><button class="btn btn-danger" onclick="event.stopPropagation();App.deleteLote(${l.id})" aria-label="Eliminar lote"><i class="ti ti-trash" aria-hidden="true"></i></button></td>
      </tr>`;
    }).join("");
  ...
  function openDetail(id) {
    ...
    $("md-titulo").textContent = l.nombre;
    $("md-pill").innerHTML = cropPill(l.cereal);
    $("md-estado").innerHTML = l.cerrado ? '<span class="badge badge-closed">Cerrado</span>' : '<span class="badge badge-open">Abierto</span>';

    $("md-nombre").value = l.nombre;
    $("md-cereal").value = l.cereal;
    $("md-hectareas").value = l.hectareas;
    $("md-participacion").value = l.participacion;
    $("md-campana").value = l.campana;
    $("md-insumos").value = l.insumos;
    $("md-labores").value = l.labores;
    $("md-tipo-alquiler").value = l.tipo_alquiler;
    $("md-alq-fijo").value = l.alquiler_fijo;
    $("md-alq-qq").value = l.alquiler_qq;
    $("md-alq-soja").value = l.alquiler_precio_soja;
    $("md-alq-pct").value = l.alquiler_pct;
    $("md-precio-esp").value = l.precio_esperado;
    $("md-gc-pct").value = l.gastos_comerciales_pct;
    $("md-rinde-real").value = l.rinde_real ?? "";
    $("md-precio-real").value = l.precio_real ?? "";
    $("md-gc-tipo").value = l.gc_tipo_real || "pct";
    $("md-gc-valor").value = l.gc_valor_real ?? "";
    $("md-tc-ars").value = l.tipo_cambio_ars ?? "";
  ...
  function closeDetail() {
    $("modal-detalle").hidden = true;
    document.body.style.overflow = "";
    ["md-comp", "md-wf", "md-sens"].forEach((k) => destroy(k));
    detailId = null;
  }

  function readDetailForm() {
    return {
      nombre: str("md-nombre").trim() || "Lote sin nombre",
      cereal: str("md-cereal"),
      campana: str("md-campana") || str("campaign"),
      hectareas: num("md-hectareas"),
      participacion: num("md-participacion"),
      insumos: num("md-insumos"),
      labores: num("md-labores"),
      tipo_alquiler: str("md-tipo-alquiler"),
      alquiler_fijo: num("md-alq-fijo"),
      alquiler_qq: num("md-alq-qq"),
      alquiler_precio_soja: num("md-alq-soja"),
      alquiler_pct: num("md-alq-pct"),
      precio_esperado: num("md-precio-esp"),
      gastos_comerciales_pct: num("md-gc-pct"),
      rinde_real: num("md-rinde-real") || null,
      precio_real: num("md-precio-real") || null,
      gc_tipo_real: str("md-gc-tipo"),
      gc_valor_real: num("md-gc-valor"),
      tipo_cambio_ars: num("md-tc-ars"),
      cerrado: !!(num("md-rinde-real") && num("md-precio-real"))
    };
  }

  function refreshDetailRent() {
    const t = str("md-tipo-alquiler");
    $("md-rent-fixed").hidden = t !== "fixed";
    $("md-rent-qq").hidden = t !== "qq";
    $("md-rent-pct").hidden = t !== "pct";
    calcDetail();
  }

  function refreshDetailGc() {
    const t = str("md-gc-tipo");
    const labels = { pct: "Gastos comerciales (%)", usd: "Gastos comerciales (USD/Ha)", ars: "Gastos comerciales (ARS/Ha)" };
    $("md-gc-label").textContent = labels[t];
    $("md-ars-row").hidden = t !== "ars";
    calcDetail();
  }

  function calcDetail() {
    const d = readDetailForm();
    const p = computeProjected(d);
    const cl = computeClose(d);

    $("md-ind-ha").textContent = fmt(p.indHa) + " Kg/Ha";
    $("md-ind-tot").textContent = fmt(p.indTot) + " Kg";

    if (cl) {
      setSigned($("md-margen-ha"), cl.margenHaNet, "USD " + fmt2(cl.margenHaNet), false);
      setSigned($("md-margen-tot"), cl.margenHaNet * d.hectareas, "USD " + fmt(cl.margenHaNet * d.hectareas), false);
    } else {
      $("md-margen-ha").textContent = "—"; $("md-margen-ha").className = "metric-value";
      $("md-margen-tot").textContent = "—"; $("md-margen-tot").className = "metric-value";
    }

    // Gráfico 1: composición
    drawDetailComposition(p.ins * p.pct, p.lab * p.pct, p.rentFinal * p.pct);
    // Gráfico 2: cascada (solo si cerrado)
    drawDetailWaterfall(d, cl);
    // Gráfico 3: sensibilidad (usa precio esperado y, si hay, rinde real; sino indiferencia como referencia)
    const rindeRef = d.rinde_real || p.indHa;
    const precioRef = d.precio_real || d.precio_esperado;
    drawDetailSensitivity(d, rindeRef, precioRef);
  }

  function drawDetailComposition(ins, lab, alq) {
    const total = ins + lab + alq;
    const vals = [ins, lab, alq];
    const labels = ["Insumos", "Labores", "Alquiler"];
    const colors = [COL.green, COL.blue, COL.amber];
    const pcts = vals.map((v) => (total > 0 ? ((v / total) * 100).toFixed(1) : "0.0"));
    $("md-leg-comp").innerHTML = labels.map((l, i) =>
      `<span><span class="legend-dot" style="background:${colors[i]}"></span>${l}: $${fmt(vals[i])} (${pcts[i]}%)</span>`).join("");
    destroy("md-comp");
    charts["md-comp"] = new Chart($("md-chart-comp"), {
      type: "doughnut",
      data: { labels, datasets: [{ data: vals.map((v) => +v.toFixed(2)), backgroundColor: colors, borderWidth: 2, borderColor: "#fff", hoverOffset: 4 }] },
      options: { ...baseOpts, cutout: "62%", plugins: { legend: { display: false }, tooltip: { callbacks: { label: (c) => ` USD ${fmt2(c.raw)}/Ha` } } } }
    });
  ...
  function drawDetailWaterfall(d, cl) {
    destroy("md-wf");
    const ctx = $("md-chart-wf");
    if (!cl) { $("md-wf-hint").hidden = false; blankChart("md-wf", ctx); return; }
    $("md-wf-hint").hidden = true;
    const pct = d.participacion / 100;
    const ib = cl.ingBruto * pct, ins = d.insumos * pct, lab = d.labores * pct, alq = cl.rentUsd * pct, g = cl.gc * pct;
    const margen = ib - ins - lab - alq - g;
    const labels = ["Ing. bruto", "Insumos", "Labores", "Alquiler", "G. com.", "Margen"];
    let run = 0; const bases = [], spans = [], colors = [];
    bases.push(0); spans.push(ib); colors.push(COL.teal); run = ib;
    [ins, lab, alq, g].forEach((v) => { bases.push(run - v); spans.push(v); colors.push(COL.red); run -= v; });
    bases.push(0); spans.push(margen); colors.push(margen >= 0 ? COL.green : COL.red);
    charts["md-wf"] = new Chart(ctx, {
      type: "bar",
      data: { labels, datasets: [
        { data: bases, backgroundColor: "transparent", stack: "w" },
        { data: spans, backgroundColor: colors, borderRadius: 4, stack: "w" }
      ...
      options: { ...baseOpts,
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: (c) => c.datasetIndex === 1 ? ` USD ${fmt2(c.raw)}/Ha` : null } } },
        scales: { x: { ticks: { color: axisColor(), font: { size: 10 }, maxRotation: 40 }, grid: { display: false } },
          y: { stacked: true, ticks: { color: axisColor(), font: { size: 10 }, callback: (v) => "$" + fmt(v) }, grid: { color: gridColor() } } }
      }
    });
  }

  function drawDetailSensitivity(d, rinde, precio) {
    destroy("md-sens");
    const ctx = $("md-chart-sens");
    const steps = [-20, -10, 0, 10, 20];
    const byPrice = steps.map((s) => estimateMargin(d, rinde, precio * (1 + s / 100)));
    const byYield = steps.map((s) => estimateMargin(d, rinde * (1 + s / 100), precio));
    $("md-leg-sens").innerHTML =
      `<span><span class="legend-dot" style="background:${COL.blue}"></span>Precio</span>` +
      `<span><span class="legend-dot" style="background:${COL.amber}"></span>Rinde</span>`;
    charts["md-sens"] = new Chart(ctx, {
      type: "line",
      data: { labels: steps.map((s) => (s > 0 ? "+" : "") + s + "%"),
        datasets: [
          { label: "Precio", data: byPrice.map((v) => +v.toFixed(0)), borderColor: COL.blue, backgroundColor: COL.blue, tension: 0.25, pointRadius: 3, borderWidth: 2 },
          { label: "Rinde", data: byYield.map((v) => +v.toFixed(0)), borderColor: COL.amber, backgroundColor: COL.amber, borderDash: [5, 4], tension: 0.25, pointRadius: 3, borderWidth: 2 }
        ]
      ...
      options: { ...baseOpts,
        ...
        scales: { x: { ticks: { color: axisColor(), font: { size: 10 } }, grid: { display: false } },
          y: { ticks: { color: axisColor(), font: { size: 10 }, callback: (v) => "$" + fmt(v) }, grid: { color: gridColor() } } }
      }
    });
  }
  ...
  async function saveDetail() {
    ...
    const d = readDetailForm();
    const btn = $("md-guardar");
    if (!online || !sb) { alert("No hay conexión con la base."); return; }
    btn.disabled = true;
    const { error } = await sb.from("lotes").update(d).eq("id", detailId);
    btn.disabled = false;
  ...
  function bindDetailInputs() {
    document.querySelectorAll("#modal-detalle input, #modal-detalle select").forEach((el) => {
      el.addEventListener("input", calcDetail);
    });
    $("modal-detalle").addEventListener("click", (e) => {
      if (e.target.id === "modal-detalle") closeDetail();
    });
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && !$("modal-detalle").hidden) closeDetail();
    });
  }
  ...
  return {
    ...
    openDetail, closeDetail, saveDetail, refreshDetailRent, refreshDetailGc
  };
})();
